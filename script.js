...
let gameStart;
let gameStarted = 0; //at first game start is false
...
function setup() {
  ...
  x = random(width); // the ball appear in the random position
}

function draw() {
  if (!gameStarted) { // if game hasn't started
    image(gameStart, 0, 0, width, height); // put game start image at canvas
  ...
  if (score > 10) { // check if the score is greater than 10
    ...
    textSize(20);
    text("Click to restart", width / 2, height / 2 + 40); // prompt to restart
    return; 
  }
  ...
  if (score < -10) { // check if the score is smaller that -10
    ...
    textAlign(CENTER, CENTER); // set text at the center
    fill(255); // the color
    text("You Lose!", width / 2, height / 2); // show "You Lose" if the score is smaller than -10
    textSize(20);
    text("Click to restart", width / 2, height / 2 + 40); // prompt to restart
    return; 
  }
...
function mouseClicked() {
  ...
  } else if (gameStarted && (score > 10 || score < -10)) { // if game already end one time
    resetGame(); // reset the game
  }
}

function resetGame() {
  score = 0; // restart the game
  y = 0; // the object will appear at top
  x = random(width); // ball in random position
  gameStarted = 0; // set gameStarted to 0 to show the start screen again
}
